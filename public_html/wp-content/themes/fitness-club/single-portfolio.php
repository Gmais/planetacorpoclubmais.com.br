...
if ( have_posts() ) {
	...
	if( $subheading != '' ) {
		$full_title .= '<br><em>' . $subheading . '</em>';		
		$data_items_split ++;
	}
	...
	$short_meta_right_html = '';
	$long_meta_right_html = '';
	...
	if ( $cf != '' ) {
		...
		for ( $i = 0; $i < count( $cf ); $i++ ) {
			...
			if ( $i < $data_items_split - 1 ) {
				$short_meta_right_html .= '<dt>' . $item_key . '</dt>';
				$short_meta_right_html .= '<dd>' . $item_value . '</dd>';
			}
			$long_meta_right_html .= '<dt>' . $item_key . '</dt>';
			$long_meta_right_html .= '<dd>' . $item_value . '</dd>';
		}
	}
					...
					<div class="boldRow ">
						<div class="rowItem col-sm-12 col-md-12 btTextLeft inherit btMiddleVertical">
							<div class="rowItemContent">
								<div class="btCloseGhost"><?php echo boldthemes_get_icon_html( 's7_e680', '#', '', 'btIcoOutlineType btIcoDefaultColor btIcoLargeSize' ); ?></div>
											...
											<div class="boldCell" data-slick="yes">
												<div class="btSlideGutter gutter">
													<div class="btSlidePane port">
														<div class="col-sm-9">
															<?php echo boldthemes_get_heading_html( $categories_html, $full_title, boldthemes_get_the_excerpt( get_the_ID() ), 'huge', $dash, '', '' ) ;; ?>
														</div><!-- /boldBlogArticle -->
														<div class="col-sm-3 btTextRight">
															<dl class="btArticleMeta">
																<?php echo wp_kses_post( $short_meta_right_html ); ?>
															</dl>
														</div>
													</div>
												</div><!-- /.boldCell-->
								...
								<div class="btInfoBar">
									<div class="btInfoBarMeta">
										...
										<p><strong><?php esc_html_e( 'Title:', 'fitness-club' ); ?></strong> <span class="btPortfolioSliderCaption"></span></p>
										<p><strong><?php esc_html_e( 'Description:', 'fitness-club' ); ?></strong> <span class="btPortfolioSliderDescription"></span></p>
									</div><!-- /boldInfoBarMeta -->
								</div><!-- /boldInfoBar -->
							</div><!-- /rowItemContent -->
						</div><!-- /rowItem -->
	...
	$content_html = str_replace( ']]>', ']]&gt;', $content_html );
		...
		if ( '' != $prev ) {
			...
			$prev_next_html .= '<span class="nbsItem"><span class="nbsDir">' . esc_html__( 'previous', 'fitness-club' ) . '</span><span class="nbsTitle">' . esc_html( $prev->post_title ) . '</span></span>';
			$prev_next_html .= '</a></h4>';
		}
		$prev_next_html .= '</div>';
		...
		if ( '' != $next ) {
			...
			$prev_next_html .= '<span class="nbsItem"><span class="nbsDir">' . esc_html__( 'next', 'fitness-club' ) . '</span><span class="nbsTitle">' . esc_html( $next->post_title ) . '</span></span>';
			$style_el = '';
