<?php
	...
	$share_html = '<div class="btIconRow">' . boldthemes_get_share_html( $permalink, 'blog', 'btIcoSmallSize', 'btIcoFilledType btIcoDefaultColor' ) . '</div>';
	...
	if ( $blog_author || $blog_date || $show_comments_number ) {
		...
		if ( $blog_author && ! $blog_side_info ) $meta_html .= '<p class="btArticleListBodyAuthor">' . boldthemes_get_post_author2( BoldThemesFramework::$author_url ) . '</p>' ;
	}
					...
					echo '<div class="rowItem col-sm-12">';
						echo '<div class="rowItemContent">';
							...
							if ( $blog_side_info ) {
								...
								if ( $avatar_html != '' ) {
									echo '<div class="asgItem avatar"><a href="' . esc_url_raw( BoldThemesFramework::$author_url ) . '">' . $avatar_html . '</a></div>';
								}
								if ( $blog_author ) {
									echo '<div class="asgItem title"><span>' . boldthemes_get_post_author2( BoldThemesFramework::$author_url ) . '</span></div>';
								}
								if ( $blog_date ) {
								...
								echo '<div class = "boldRow">';
									...
									echo '<div class="rowItem btTextLeft ' . esc_attr( $right_col_class ) . '">';
										echo '<div class="rowItemContent">';
											...
											echo '<div class="btShareArticle">' . $share_html . '</div>';
