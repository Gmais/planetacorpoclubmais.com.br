...
if ( get_option( 'woocommerce_enable_review_rating' ) !== 'no' ) {
	...
	if ( wc_product_sku_enabled() && ( $product->get_sku() || $product->is_type( 'variable' ) ) ) {
		...
		$sku = $product->get_sku() ? $product->get_sku() : esc_html__( 'N/A', 'fitness-club' );
		$subtitle .= '<span class = "btProductSKU product_meta">' . esc_html__( 'SKU:', 'fitness-club' ) . ' <span class="sku">' . $sku . '</span></span>';
	}
