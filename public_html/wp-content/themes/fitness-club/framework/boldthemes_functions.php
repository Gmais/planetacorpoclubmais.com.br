...
if ( ! function_exists( 'boldthemes_pagination' ) ) {
	function boldthemes_pagination() {
		...
		$prev = get_previous_posts_link( esc_html__( 'Newer Posts', 'fitness-club' ) );
		$next = get_next_posts_link( esc_html__( 'Older Posts', 'fitness-club' ) );
...
if ( ! function_exists( 'boldthemes_theme_comment' ) ) {
	function boldthemes_theme_comment( $comment, $args, $depth ) {
		...
		<li <?php comment_class(); ?> id="comment-<?php comment_ID(); ?>">
			<p><?php esc_html_e( 'Pingback:', 'fitness-club' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( esc_html__( '(Edit)', 'fitness-club' ), '<span class="edit-link">', '</span>' ); ?></p>
		<?php
				break;
			default :
				...
				<div class="commentTxt">
					<div class="vcard divider">
						<?php
							...
							echo '<p class="posted">' . sprintf( esc_html__( '%1$s at %2$s', 'fitness-club' ), get_comment_date(), get_comment_time() ) . '</p>';
							if ( $rating > 0 && get_option( 'woocommerce_enable_review_rating' ) == 'yes' ) { ?>
								<div itemprop="reviewRating" itemscope itemtype="http://schema.org/Rating" class="star-rating" title="<?php echo sprintf( esc_html__( 'Rated %d out of 5', 'fitness-club' ), $rating ) ?>">
									<span style="width:<?php echo wp_kses_post( ( $rating / 5 ) * 100 ); ?>%"><strong itemprop="ratingValue"><?php echo wp_kses_post( $rating ); ?></strong> <?php esc_html_e( 'out of 5', 'fitness-club' ); ?></span>
								</div>
							<?php }
						?>
					</div>
					...
					<?php if ( '0' == $comment->comment_approved ) : ?>
						<p class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'fitness-club' ); ?></p>
					<?php endif; ?>
					...
					<div class="comment">
						...
						if ( comments_open() ) {
							echo '<p class="reply">';
								comment_reply_link( array_merge( $args, array( 'reply_text' => esc_html__( 'Reply', 'fitness-club' ), 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) );
							echo '</p>';
						}
						edit_comment_link( esc_html__( 'Edit', 'fitness-club' ), '<p class="edit-link">', '</p>' ); ?>
					</div>
				</div>
...
if ( ! class_exists( 'RWMB_BoldThemesText_Field' ) && class_exists( 'RWMB_Field' ) ) {
	class RWMB_BoldThemesText_Field extends RWMB_Field {
		...
		static function html( $meta, $field ) {	
			...
			if ( ! $meta ) $meta = ''; // 8.x fix: PHP Deprecated: htmlspecialchars(): Passing null to parameter #1 ($string) of type string is deprecated
			return sprintf(
				'<input type="hidden" class="rwmb-text" name="%s" id="%s" value="%s" placeholder="%s" %s>%s',
				$field['field_name'],
				$field['id'],
				esc_attr( htmlspecialchars( $meta ) ),
				$field['placeholder'],
				'',
...
if ( ! function_exists( 'boldthemes_get_posts_array' ) ) {
	function boldthemes_get_posts_array( $recent_posts_q = '', $post_type = 'blog', $sticky_arr = array() ) {
...
if ( ! function_exists( 'boldthemes_get_posts_array_item' ) ) {
	function boldthemes_get_posts_array_item( $post_type = 'blog', $post_id = 0, $post_author = 0 ) {
...
if ( ! function_exists( 'boldthemes_set_global_uri' ) ) {
	function boldthemes_set_global_uri() {
		...
		$data .= 'window.boldthemes_text.previous = \'' . esc_html__( 'previous', 'fitness-club' ) . '\';';
		$data .= 'window.boldthemes_text.next = \'' . esc_html__( 'next', 'fitness-club' ) . '\';';
		return $data;
	}
}
...
if ( ! function_exists( 'boldthemes_get_post_author' ) ) {
	function boldthemes_get_post_author( $author_url = false ) {
		...
		return '<a href="' . esc_url_raw( $author_url ) . '" class="btArticleAuthor">' . esc_html__( 'by', 'fitness-club' ) . ' ' . esc_html( get_the_author_meta( 'display_name', $post_author_id )  ) . '</a>';
	}
}
		...
		if ( ! is_404() && ! is_front_page() ) {
			...
			if ( ! $simple ) {
				...
				if ( ! is_singular() || is_page() ) {
					$output .= '<li><a href="' . esc_url_raw( $home_link ) . '">' . esc_html__( 'Home', 'fitness-club' ) . '</a></li>';
				}
			} else {
				if ( ! is_singular() || is_page() ) {
					$output .= '<a href="' . esc_url_raw( $home_link ) . '">' . esc_html__( 'Home', 'fitness-club' ) . '</a>';
				}
			}
			...
			if ( is_home() ) {
				...
				$output .= $item_prefix . esc_html__( 'Portfolio', 'fitness-club' ) . $item_suffix;
				...
				$output .= $item_prefix . esc_html__( 'Category', 'fitness-club' ) . $item_suffix;
				...
				$output .= $item_prefix . esc_html__( 'Tag', 'fitness-club' ) . $item_suffix;
				
				$subtitle = '';
		  
			} else if ( is_author() ) {
			
				$output .= $item_prefix . esc_html__( 'Author', 'fitness-club' ) . $item_suffix;
				...
				$output .= $item_prefix . esc_html__( 'Search', 'fitness-club' ) . $item_suffix;
