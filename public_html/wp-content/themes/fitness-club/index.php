...
if ( have_posts() ) {
	...
	while ( have_posts() ) {
		...
		BoldThemesFramework::$author_url = get_author_posts_url( get_the_author_meta( 'ID' ) );
	...
	if ( is_search() ) { ?>
		<article class="btNoSearchResults boldSection gutter bottomSemiSpaced topSemiSpaced ">
			<div class="port">
			<?php echo boldthemes_get_heading_html( '', esc_html__( 'We are sorry, no results for: ', 'fitness-club' ) . get_search_query(), '<a href="' . site_url() . '">' . esc_html__( 'Back to homepage', 'fitness-club' ) . '</a>', 'medium', '', '', '' ) ?>
			</div>
		</article>
	<?php }
}
