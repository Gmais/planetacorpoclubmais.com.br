...
/**
 * Plugin Name: Fitness Club Plugin
...
class bt_column_inner {
	...
	static function handle_shortcode( $atts, $content ) {
		...
		if ( $highlight == 'shadow' ) {
			$class[] = 'btHighlightShadow';
		} else if ( $highlight == 'border' ) {
			$class[] = 'btHighlightBorder';
		} else if ( $highlight == 'shadow_border' ) {
			$class[] = 'btHighlightShadow';
			$class[] = 'btHighlightBorder';
		}

		if ( $highlight_border_color != '' ) {
			$el_style .= 'border-color:' . $highlight_border_color . ';';
		}
...
class bt_header {
	...
	static function handle_shortcode( $atts, $content ) {
		...
		if ( $color == 'accent1' ) {
			$class = 'btHeaderAccent1';
		} else if ( $color == 'accent2' ) {
			$class = 'btHeaderAccent2';
		} else if ( $color == 'accent3' ) {
			$class = 'btHeaderAccent3';
		} else if ( $color == 'accent4' ) {
			$class = 'btHeaderAccent4';
		}
...
class bt_service {
	...
	static function handle_shortcode( $atts, $content ) {
		...
		$output = '<div class="servicesItem ' . ' ' . 'btServices' . ucfirst( $icon_color ) . ' ' . $icon_size . 'Icon ' . $el_class . '"' . $style_attr . '>';
			$output .= '<div class="sIcon">';
				$output .= boldthemes_get_icon_html( $icon, $url, '', $icon_size . ' ' . $icon_type . ' ' . 'bt' . ucfirst( $icon_color ) . 'Icon' );
			$output .= '</div>';
			if ( $headline != '' || $text != '') {
...
class bt_countdown {
	...
	static function handle_shortcode( $atts, $content ) {
		...
		$output = '<div class="' . implode( ' ', $el_class ) . '"' . $style_attr . '>';
			$output .= '<div class="btCountdownHolder" data-init-seconds="' . $init_seconds . '" data-init-target-seconds="' . esc_attr( $target ) . '">';
...
class bt_icon {
	...
	static function handle_shortcode( $atts, $content ) {
		...
		$output = boldthemes_get_icon_html( $icon, $url, $icon_title, $icon_type . ' ' . $icon_size . ' ' . 'bt' . ucfirst( $icon_color ) . 'Icon' . $el_class, $target, $el_style );
...
class bt_grid {
		...
		foreach( $data as $post ) {
			...
			if ( $post_type == 'portfolio' ) {
				...
				if ( boldthemes_get_option( 'blog_author' ) ) {
					...
					$author = '<a href="' . esc_url_raw( $author_url ) . '" class="btArticleAuthor">' . __( 'by', 'bt_plugin' ) . ' ' . $post['author_name'] . '</a>';		
				}
			...
			if ( $grid_type == 'classic' ) {
				...
				if ( $post_type == 'portfolio' ) {
					$share_html = boldthemes_get_share_html( $post['permalink'], 'pf', 'btIcoSmallSize', 'btIcoDefaultColor btIcoFilledType' );
					$dash = $use_dash ? 'bottom' : '';
				} else {
					$share_html = boldthemes_get_share_html( $post['permalink'], 'blog', 'btIcoSmallSize', 'btIcoDefaultColor btIcoFilledType' );
					$dash = $use_dash ? 'bottom' : '';
				}
	...
	static function handle_shortcode( $atts, $content ) {
		...
		if ( $category_filter == 'yes' ) {
			...
			$output .= '<span class="btCatFilterItem all" data-slug=""><b>' . __( 'All', 'bt_plugin' ) . '</b></span>';
			foreach ( $cats as $cat ) {
				$output .= '<span class="btCatFilterItem" data-slug="' . $cat->slug . '"><b>' . $cat->name . '</b></span>';
			}
			$output .= '</div>';
		}
...
class bt_latest_posts {
	...
	static function handle_shortcode( $atts, $content ) {
		...
		$img_size = 'boldthemes_small_rectangle';
		$dash = 'bottom';
		$indent_class = '';
...
// [bt_events_week]
class bt_events_week {
    static function init() {
			add_action( 'plugins_loaded', array(  __CLASS__, 'verify_installed' ), 2 );
			add_shortcode( 'bt_events_week', array( __CLASS__, 'handle_shortcode' ) );
			add_action( 'wp_ajax_bt_this_week', array( __CLASS__, 'bt_this_week_callback' ) );
			add_action( 'wp_ajax_nopriv_bt_this_week', array( __CLASS__, 'bt_this_week_callback' ) );           
    }

	static function verify_installed() {
		if ( ! class_exists( 'Tribe__Events__Main' ) or ! defined( 'Tribe__Events__Main::VERSION' )) {
			add_action( 'admin_notices', array(  __CLASS__, 'show_not_installed_message' ) );
		}
	}

	static function show_not_installed_message() {
		if ( current_user_can( 'activate_plugins' ) ) {
			$url = 'plugin-install.php?tab=plugin-information&plugin=the-events-calendar&TB_iframe=true';
			$title = __( 'The Events Calendar', 'bt_plugin' );
			echo '<div class="error"><p>' . sprintf( __( 'To begin using events week shortcode, please install and activate the latest version of <a href="%s" class="thickbox" title="%s">The Events Calendar</a> and add events.', 'tec-shortcode' ), esc_url( $url ), esc_attr( $title ) ) . '</p></div>';
		}
	}

    static function handle_shortcode( $atts, $content ) {
		if ( ! class_exists( 'Tribe__Events__Main' ) or ! defined( 'Tribe__Events__Main::VERSION' )) {
			$output =  __( 'To begin using events week shortcode, please install and activate the latest version of The Events Calendar plugin and add events.', 'bt_plugin' );
			return $output;			
			die();
		}


		require_once( 'week/week.php' );

		extract( shortcode_atts( array(
				'start_date'		 => date( 'Y-m-d H:i:s'),
				'events_limit'		 => 5,
				'display_date_format'=> 'D jS',
				'start_of_week'		 => 1,
				'el_class'			 => '',
				'el_style'			 => ''
		), $atts, 'bt_events_week' ) );
		...
		wp_enqueue_script( 
			'boldthemes_events_week',
			plugin_dir_url( __FILE__ ) . 'week/resources/js/events-week.min.js',
			array( 'jquery' ),
			'',
			true
		);

		$style_url = plugin_dir_url( __FILE__ ) . 'week/resources/css/events-week-full.css';
		wp_enqueue_style( 'boldthemes-events-week-style', $style_url, array(), false );

		$widget_data = array( 'ajaxurl' => admin_url( 'admin-ajax.php', ( is_ssl() ? 'https' : 'http' ) ) );
		wp_localize_script( 'boldthemes_events_week', 'bt_this_week', $widget_data );
		...
		$bt_week_query_vars['start_date']			= bt_get_this_week_first_week_day( $start_date, $start_of_week );
		$bt_week_query_vars['end_date']				= bt_get_this_week_last_week_day( $bt_week_query_vars['start_date'] );
		$bt_week_query_vars['count']				= isset( $events_limit ) ? $events_limit : 5;
		$bt_week_query_vars['layout']				= 'horizontal';
		$bt_week_query_vars['start_of_week']		= $start_of_week;
		$bt_week_query_vars['display_date_format']	= $display_date_format;

		$bt_week_query_vars['loading_gif']			= plugin_dir_url( __FILE__ ) . 'week/resources/images/bt-events-loading.gif' ;

		$bt_week_template_vars  = bt_this_week_template_vars( $bt_week_query_vars );
		$bt_week_data_attrs		= bt_this_week_data_attr( $bt_week_query_vars );

		ob_start();
		echo '<div class="btEventsWeek ' . $el_class . '" ' . $style_attr . '>';
		include 'week/content.php';
		echo '</div>';
		return ob_get_clean();
    }
	...
	/**
	 * bt_events_week - Ajax Change Week
	 *
	 */
	static function bt_this_week_callback() {
		...
		if ( isset( $_POST['start_date'] ) && isset( $_POST['count'] ) ) {
			...
			$events_limit	= isset( $_POST['count'] )	? $_POST['count']	: 5;

			$bt_week_query_vars['start_date']			= bt_get_this_week_first_week_day( $_POST['start_date'], $_POST['start_of_week'] );
			$bt_week_query_vars['end_date']				= bt_get_this_week_last_week_day( $_POST['start_date'] );
			$bt_week_query_vars['count']				= $events_limit;
			$bt_week_query_vars['layout']				= $_POST['layout'];
			$bt_week_query_vars['start_of_week']		= $_POST['start_of_week'];
			$bt_week_query_vars['display_date_format']	= $_POST['display_date_format'];
		...
		apply_filters( 'bt_events_ajax_response', $response );
	...
	if ( function_exists( 'bt_rc_map' ) ) {
		...
		bt_rc_map( 'bt_image', array( 'name' => __( 'Image', 'bt_plugin' ), 'description' => __( 'Single image', 'bt_plugin' ), 'container' => 'vertical', 'accept' => array( 'bt_header' => true, 'bt_text' => true, 'bt_hr' => true, 'bt_icons' => true, 'bt_button' => true ), 'icon' => 'bt_bb_icon_bt_bb_image', 
			'params' => array(
				...
				array( 'param_name' => 'shape', 'type' => 'dropdown', 'heading' => __( 'Shape', 'bt_plugin' ), 'preview' => true, 'group' => __( 'Design', 'bold-builder' ),
					'value' => array(
		...
		bt_rc_map( 'bt_section', array( 'name' => __( 'Section', 'bt_plugin' ), 'description' => __( 'Basic root element', 'bt_plugin' ), 'root' => true, 'container' => 'vertical', 'accept' => array( 'bt_row' => true ), 'toggle' => true, 'show_settings_on_create' => false,
			'params' => array( 
				...
				array( 'param_name' => 'skin', 'type' => 'dropdown', 'heading' => __( 'Skin', 'bt_plugin' ), 'preview' => true, 'group' => __( 'Design', 'bold-builder' ),
					'value' => array(
			...
			'params' => array(
				...
				array( 'param_name' => 'highlight', 'type' => 'dropdown', 'heading' => __( 'Cell Highlight', 'bt_plugin' ), 'group' => __( 'Design', 'bold-builder' ),
					'value' => array(
						...
						__( 'Shadow', 'bt_plugin' ) => 'shadow',
						__( 'Border', 'bt_plugin' ) => 'border',
						__( 'Shadow + Border', 'bt_plugin' ) => 'shadow_border'
				) ),
				array( 'param_name' => 'highlight_border_color', 'type' => 'colorpicker', 'heading' => __( 'Cell Highlight Border Color', 'bt_plugin' ), 'group' => __( 'Design', 'bold-builder' ) ),
		...
		bt_rc_map( 'bt_column_inner', array( 'name' => __( 'Inner Column', 'bt_plugin' ), 'description' => __( 'Inner Column element', 'bt_plugin' ), 'icon' => 'bt_bb_icon_bt_bb_row_inner', 'width_param' => 'width', 'container' => 'vertical', 'accept' => array( 'bt_section' => false, 'bt_row' => false, 'bt_column' => false, '_content' => false, 'bt_client' => false, 'bt_icon' => false,  'bt_tabs_items' => false, 'bt_accordion_items' => false, 'bt_slider_item' => false ), 'accept_all' => true, 'toggle' => true,
			'params' => array(
				...
				array( 'param_name' => 'highlight_border_color', 'type' => 'colorpicker', 'heading' => __( 'Cell Highlight Border Color', 'bt_plugin' ) ),
				array( 'param_name' => 'animation', 'type' => 'dropdown', 'heading' => __( 'Animation', 'bt_plugin' ), 'preview' => true,
					'value' => array(
						__( 'No Animation', 'bt_plugin' ) => 'no_animation',
						__( 'Fade In', 'bt_plugin' ) => 'animate animate-fadein',
		...
		bt_rc_map( 'bt_header', array( 'name' => __( 'Header', 'bt_plugin' ), 'description' => __( 'Header element', 'bt_plugin' ), 'icon' => 'bt_bb_icon_bt_bb_headline', 'highlight' => true,
			'params' => array( 
				...
				array( 'param_name' => 'color', 'type' => 'dropdown', 'heading' => __( 'Color', 'bt_plugin' ),
					'value' => array(
						__( 'Default', 'bt_plugin' ) => 'default',
						__( 'Accent 1', 'bt_plugin' ) => 'accent1',
						__( 'Accent 2', 'bt_plugin' ) => 'accent2',
						__( 'Accent 3', 'bt_plugin' ) => 'accent3',
						__( 'Accent 4', 'bt_plugin' ) => 'accent4'
				) ),				
				array( 'param_name' => 'el_class', 'type' => 'textfield', 'heading' => __( 'Extra Class Name(s)', 'bt_plugin' ), 'group' => __( 'Custom', 'bold-builder' ) ),
				array( 'param_name' => 'el_style', 'type' => 'textfield', 'heading' => __( 'Inline Style', 'bt_plugin' ), 'group' => __( 'Custom', 'bold-builder' ) )
			) )
		...
		bt_rc_map( 'bt_tabs', array( 'name' => __( 'Tabs', 'bt_plugin' ), 'description' => __( 'Tabs container', 'bt_plugin' ), 'icon' => 'bt_bb_icon_bt_bb_tabs', 'container' => 'vertical', 'accept' => array( 'bt_tabs_items' => true ), 'show_settings_on_create' => false ));
		...
		bt_rc_map( 'bt_accordion', array( 'name' => __( 'Accordion', 'bt_plugin' ), 'description' => __( 'Accordion container', 'bt_plugin' ), 'icon' => 'bt_bb_icon_bt_bb_accordion', 'container' => 'vertical', 'accept' => array( 'bt_accordion_items' => true ), 'show_settings_on_create' => false ));
		...
		bt_rc_map( 'bt_gmaps', array( 'name' => __( 'Google Maps', 'bt_plugin' ), 'description' => __( 'Google Maps with marker on specified coordinates', 'bt_plugin' ), 'container' => 'vertical', 'icon' => 'bt_bb_icon_bt_bb_google_maps_location', 'accept' => array( 'bt_header' => true, 'bt_button' => true, 'bt_counter' => true, 'bt_icons' => true, 'bt_image' => true, 'bt_text' => true, 'bt_hr' => true, 'bt_dropdown' => true, 'bt_service' => true ),
			'params' => array(
		...
		*/	
	    ...
	    bt_rc_map( 'bt_events_week', array( 'name' => __( 'Events week view', 'bt_plugin' ), 'description' => __( 'Events week view', 'bt_plugin' ), 'icon' => 'bt_bb_icon_bt_bb_events_week',
			'params' => array( 
				array( 'param_name' => 'start_date', 'type' => 'textfield', 'heading' => __( 'Date of current week', 'bt_plugin' ), 'description' => __( 'YYYY-mm-dd, e.g. 2015-02-22' ), 'preview' => true ),
				array( 'param_name' => 'events_limit', 'type' => 'textfield', 'heading' => __( 'Number of events to show per day', 'bt_plugin' ), 'preview' => true ),				
				array( 'param_name' => 'start_of_week', 'type' => 'dropdown', 'heading' => __( 'Start day of week', 'bt_plugin' ),
					'value' => array(
						__( 'Sunday', 'bt_plugin' )		=> '0',
						__( 'Monday', 'bt_plugin' )		=> '1',
						__( 'Tuesday', 'bt_plugin' )	=> '2',
						__( 'Wednesday', 'bt_plugin' )	=> '3',
						__( 'Thursday', 'bt_plugin' )	=> '4',
						__( 'Friday', 'bt_plugin' )		=> '5',
						__( 'Saturday', 'bt_plugin' )	=> '6'
				) , 'preview' => true ),
				array( 'param_name' => 'display_date_format', 'type' => 'textfield', 'heading' => __( 'Week Day Format', 'bt_plugin' ), 'description' => __( 'D jS, e.g. Wed 15th ' ), 'preview' => true ),
				array( 'param_name' => 'el_class', 'type' => 'textfield', 'heading' => __( 'Extra Class Name(s)', 'bt_plugin' ), 'group' => __( 'Custom', 'bold-builder' ) ),
				array( 'param_name' => 'el_style', 'type' => 'textfield', 'heading' => __( 'Inline Style', 'bt_plugin' ), 'group' => __( 'Custom', 'bold-builder' ) )
			) )
		);	
...
if ( ! class_exists( 'BT_Icon_Widget' ) ) {
	...
	class BT_Icon_Widget extends WP_Widget {
		...
		public function widget( $args, $instance ) {
			...
			if ( $color != '' ) {
				if ( $color == 'accent1' ) {
					$icon_class = 'btIcoAccentColor1';
				} else if ( $color == 'accent2' ) {
					$icon_class = 'btIcoAccentColor2';
				} else if ( $color == 'accent3' ) {
					$icon_class = 'btIcoAccentColor3';
				} else if ( $color == 'accent4' ) {
					$icon_class = 'btIcoAccentColor4';
				}
			}
			
			if ( $style == 'circled' ) {
				$icon_class = $icon_class . ' ' . 'btIcoCircled';
			}
		...
		public function form( $instance ) {
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'color' ) ); ?>"><?php _e( 'Color:', 'bt_plugin' ); ?></label> 
				<select class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'color' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'color' ) ); ?>">
					<?php
					$target_arr = array( __( 'Default', 'bt_plugin' ) => 'default', __( 'Accent 1', 'bt_plugin' ) => 'accent1', __( 'Accent 2', 'bt_plugin' ) => 'accent2', __( 'Accent 3', 'bt_plugin' ) => 'accent3', __( 'Accent 4', 'bt_plugin' ) => 'accent4' );
			...
			<p>
				...
				<select class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'style' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'style' ) ); ?>">
					<?php
					$target_arr = array( __( 'Default', 'bt_plugin' ) => 'default', __( 'Circled', 'bt_plugin' ) => 'circled' );
					foreach( $target_arr as $key => $value ) {
						if ( $value == $style ) {
							echo '<option value="' . $value . '" selected>' . $key . '</option>';
						} else {
			...
			<p>
				...
				<select class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'target' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'target' ) ); ?>">
					<option value=""></option>;
					<?php
					$target_arr = array( __( 'Self', 'bt_plugin' ) => '_self', __( 'Blank', 'bt_plugin' ) => '_blank', __( 'Parent', 'bt_plugin' ) => '_parent', __( 'Top', 'bt_plugin' ) => '_top' );
					foreach( $target_arr as $key => $value ) {
						if ( $value == $target ) {
							echo '<option value="' . $value . '" selected>' . $key . '</option>';
						} else {
...
if ( ! class_exists( 'BT_Time_Widget' ) ) {
	...
	class BT_Time_Widget extends WP_Widget {
		...
		public function widget( $args, $instance ) {
			...
			echo '<span id="' . $this->container_id . '" class="btIconWidget"><span class="btIconWidgetIcon">' . boldthemes_get_icon_html( 's7_e66b', '', '', 'btIcoDefaultType btIcoAccentColor2', '' ) . '</span><span class="btIconWidgetContent"><span class="btIconWidgetTitle">' . $this->place_name . '</span><span class="btIconWidgetText"></span></span></span>';
		}
